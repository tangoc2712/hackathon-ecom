...
\restrict rQpuUKPYvYDG3hAbVJQzKuRtoyfffkhNaar8y7UZmjkIj6IeAvHpKrAWIF7eqcc
...
SELECT pg_catalog.set_config('search_path', 'shopspot', false);
...
CREATE SCHEMA shopspot;


--
-- Name: SCHEMA shopspot; Type: COMMENT; Schema: -; Owner: -
--

COMMENT ON SCHEMA shopspot IS 'standard public schema';
...
CREATE TABLE shopspot._prisma_migrations (
...
CREATE TABLE shopspot.order_items (
...
CREATE TABLE shopspot.products (
...
COPY shopspot._prisma_migrations (id, checksum, finished_at, migration_name, logs, rolled_back_at, started_at, applied_steps_count) FROM stdin;
...
COPY shopspot.coupons (id, code, amount, created_at, updated_at) FROM stdin;
\.
...
COPY shopspot.order_items (order_item_id, order_id, product_id, quantity, unit_price, total_price) FROM stdin;
\.
...
COPY shopspot.orders (order_id, user_id, status, order_total, currency, created_at, updated_at, shipping_info, subtotal, tax, shipping_charges, discount) FROM stdin;
\.
...
COPY shopspot.products (product_id, sku, name, description, price, currency, created_at, updated_at, is_active, sale_price, category_id, product_url, stock, photos, colors, sizes, materials, care, photo_public_id, featured, category_name) FROM stdin;
...
COPY shopspot.users (user_id, email, password_hash, full_name, phone, address, created_at, updated_at, is_active, img_url, role_id, date_of_birth, job, gender, uid, photo_url, provider, role) FROM stdin;
...
ALTER TABLE ONLY shopspot._prisma_migrations
...
ALTER TABLE ONLY shopspot.coupons
...
ALTER TABLE ONLY shopspot.order_items
...
ALTER TABLE ONLY shopspot.orders
...
ALTER TABLE ONLY shopspot.products
...
ALTER TABLE ONLY shopspot.users
...
CREATE UNIQUE INDEX coupons_code_key ON shopspot.coupons USING btree (code);
...
CREATE UNIQUE INDEX products_sku_key ON shopspot.products USING btree (sku);
...
CREATE UNIQUE INDEX users_email_key ON shopspot.users USING btree (email);
...
CREATE UNIQUE INDEX users_uid_key ON shopspot.users USING btree (uid);
...
ALTER TABLE ONLY shopspot.order_items
    ADD CONSTRAINT order_items_order_id_fkey FOREIGN KEY (order_id) REFERENCES shopspot.orders(order_id) ON UPDATE CASCADE ON DELETE SET NULL;
...
ALTER TABLE ONLY shopspot.order_items
    ADD CONSTRAINT order_items_product_id_fkey FOREIGN KEY (product_id) REFERENCES shopspot.products(product_id) ON UPDATE CASCADE ON DELETE SET NULL;
...
ALTER TABLE ONLY shopspot.orders
    ADD CONSTRAINT orders_user_id_fkey FOREIGN KEY (user_id) REFERENCES shopspot.users(uid) ON UPDATE CASCADE ON DELETE SET NULL;
...
\unrestrict rQpuUKPYvYDG3hAbVJQzKuRtoyfffkhNaar8y7UZmjkIj6IeAvHpKrAWIF7eqcc
