...
const AddFeaturedProductModal: React.FC<AddFeaturedProductModalProps> = ({
    ...
    const handleAddFeature = async (productId: string) => {
        try {
            ...
            const updatedProduct = allProductsData.products.find(product => product.product_id === productId);
            if (updatedProduct) {
                    ...
                    {filteredProducts.map(product => (
                        <tr key={product.product_id}>
                            ...
                            <td className="py-2 px-4 border-b">
                                <img src={product.photo || (product.photos && product.photos.length > 0 ? product.photos[0] : '')} alt={product.name} className="w-16 h-16" />
                            </td>
                            <td className="py-2 px-4 border-b">
                                <button
                                    className="bg-blue-500 text-white px-4 py-2 rounded"
                                    onClick={() => handleAddFeature(product.product_id)}
                                >
                                    Add to Featured
                                </button>
                            </td>
